...
string lectura(string ar){
    ...
    archivo.close();
   palabra=b.a.decodifica2(palabra);
    return  palabra;
}
...
void escribir(string info, string ar){    //escribir una iformacion en un archivo deseado
    ...
    ar=t+ar;
    info+='\n';
    info=b.a.pasarbin(info);
    info=b.a.codifica2(info);
...
{
    string peliculas=lectura("peliculas.bin");
   ...
   duracions=to_string(duracion);
    ...
    makesill(I,z);
    madepel(peliculas);
    escribir(peliculas,"peliculas.bin");
...
void agregarasiento(){  // agregar un nuevo tipo de asiento
    string asientos=lectura("asientos.bin");   // cargo el string
    string nombre;
    string costo;
    int id=knowid(asientos);       // utilizo mi funcion para saber cual es el ultimo id usado
     ...
     escribir(asientos,"asientos.bin");
}
...
int tipoasiento(int IDas,string *vents){     // elegir tipo de siento
    string info=lectura("asientos.bin");
    int tam=info.length();
    int pres,dinerous=0;
    string pelicula,num;
    for(int i=0;i<tam;i++){
...
bool peliculafull(int sele3){
    string info=lectura("peliculas.bin");
    int tam=info.length();
    int anc,disp;
    string pelicula,num;
    for(int i=0;i<tam;i++){
    ...
    for(int i=0;i<tam;i++){
        ...
        int nume=IDre( pelicula);    //conseguir ID de la linea
         if(sele3==nume){             // si coincide con la pelicula donde se compro haga..
          ...
          info=b.a.pasarbin(info);
          info=b.a.codifica2(info);
          ofstream archivo2("../archivo/peliculas.bin");
          archivo2<<info;
          archivo2.close();
...
void agregarusuario()
{
    string usuario=lectura("sudo.bin");
    string cedula,clave;
...
void newestreno()
{
    string peliculas=lectura("estrenos.bin");
    int I=knowid(peliculas);          // uso la funcion creada para saber mi ultimo ID
   ...
   duracions=to_string(duracion);
    ...
    peliculas += parte;
    madepel(peliculas);
    escribir(peliculas,"estrenos.bin");
